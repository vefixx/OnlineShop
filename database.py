...
def add_product_in_cart(user_login, product_id, old_products):
    cursor = conn.cursor()
    if old_products:
        total_products = total_product_in_cart(old_products)  # Преобразуем строку в список товаров
        if str(product_id) in total_products:  # Проверяем наличие product_id в списке товаров
            return False  # Если товар уже есть в корзине, возвращаем False
